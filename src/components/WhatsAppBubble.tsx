...
export default function WhatsAppBubble({
  ...
  return (
    <div
      ref={bubbleRef}
      className={`flex ${isUser ? 'justify-end' : 'justify-start'} animate-slide-up mb-1 group`}
    >
      <div className={`flex items-start gap-1 max-w-[85%] sm:max-w-[70%] ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
        <div
          className={`
            relative px-3 py-2 rounded-2xl shadow-sm flex-1
            ${isUser
        ...
        {/* Copy button – shown on hover or when showCopy is true */}
        {!isLoading && showCopy && message && (
          <div className={`flex-shrink-0 self-center opacity-0 group-hover:opacity-100 transition-opacity ${isUser ? 'mr-1' : 'ml-1'}`}>
